...
def train_open_task_exp2():
    ...
    with open('EXP3.pkl', 'rb') as file:
        data = pickle.load(file)
    vectors_exp3 = read_matrix('vectors_243sentences.GV42B300.average.txt', sep=" ")
    passages_exp3 = [item[0][0] for item in data['keyPassages']]
    topic_exp3 = [item[0] for sublist in data['keyPassageCategory'] for item in sublist.flatten()]
    imaging_data_exp3 = data['Fmridata']
    ...
    for reg_type in regression_types:
        ...
        decoded_test_vectors = [np.dot(stimuli, decoder) for stimuli in imaging_data_exp3]
        accuracy_dict = {}
        # Accuracy analysis
        for i, decoded_vec in enumerate(decoded_test_vectors):
            ...
            accuracy_dict[topic]['distance_sum'] += distance_of_sentence(decoded_vec, vectors_exp3, i)
        accuracy_list = []
        for topic in topic_exp3:
    ...
    print(results_dict)
    with open('results_open_task_2_data3.pkl', 'wb') as file:
        pickle.dump(results_dict, file)

def plot_open_task_exp2():
    with open('results_open_task_2_data3.pkl', 'rb') as file:
        data = pickle.load(file)
    ...
    ax.set_title('Normalized Rank Accuracy by Topic and Regression Method (Experiment 3 data)')
    ax.legend()
